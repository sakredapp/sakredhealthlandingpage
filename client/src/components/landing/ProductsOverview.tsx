 ...
 * Deliberately icon-free: type does the work, the pillar chip carries the accent.
 *
          ...
          {PRODUCTS.map((p, i) => (
            <Reveal key={p.slug} delay={stagger(i)}>
              <Link href={`/products/${p.slug}`} className="block h-full">
                <GlowCard className="group h-full bg-white rounded-2xl border border-[#E8E4DC] p-6 sm:p-7 hover:border-[#C5A059]/50 hover:shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] transition-all duration-300 flex flex-col">
                  <div className="flex items-center justify-between gap-4 mb-3">
                    <span className="text-[11px] font-medium uppercase tracking-[0.18em] text-[#C5A059]">
                      {p.pillar}
                    </span>
                    <span className="h-px flex-1 bg-gradient-to-r from-[#C5A059]/40 to-transparent" />
                  </div>
                  <h3 className="font-display font-semibold text-[#2C2C2C] text-xl mb-1">
                    {p.title}
                  </h3>
